...
@section( 'content' )
    <div class="container-fluid">
        ...
        <div class="row">
            <div class="col-md-12">
                <a class="btn btn-info" href="{{ route('app.projects.create') }}" role="button"><i class="fa fa-plus-circle" aria-hidden="true"></i>  Novo Projeto</a>
            </div>
        </div>
                ...
                @if( count($projects) > 0 )
                    <table class="table table-striped table-bordered">
                        ...
                        @foreach($projects as $i => $project)
                            <tr>
                                <td>{{$i+1}}</td>
                                <td><a href="{{url('app/projects/'.$project->id)}}">{{$project->name}}</a></td>
                                ...
                                <td>@if($project->user->id == Auth::user()->id)
                                    <a href="{{ url('app/projects/'.$project->id.'/edit/') }}" class="btn btn-warning btn-xs">
